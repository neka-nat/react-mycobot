...
export class Command {
  private make_command(genre: number, data: number[]) {
  ...
  private isFrameHeader(data: Uint8Array, pos: number): boolean {
    return (
  ...
  private processReceive(data: Uint8Array, genre: number): Int16Array {
    if (data.length === 0) return new Int16Array();
    const index = data.findIndex((_, i, array) => this.isFrameHeader(array, i));
    if (index === -1) return new Int16Array();

    const data_len = data[index + 2] - 2;
    const cmd_id = data[index + 3];
    if (cmd_id !== genre) return new Int16Array();
